...
reprobados = df[df[modulos].lt(5).any(axis=1)]

reprobados.to_csv('alumnos_reprobados.csv', index=False)
print(f"Se han exportado {len(reprobados)} alumnos con al menos un módulo reprobado.")
print(reprobados[['Nombre'] + modulos].to_string(index=False))
