...
csv_path = os.path.join(script_dir, 'nota_minima_modulos.csv')
...
nota_minima = df[modulos].min()
nota_minima_df = nota_minima.reset_index()
nota_minima_df.columns = ['Modulo', 'Nota Minima']
nota_minima_df.to_csv(csv_path, index=False)

print(f"Nota mínima por módulo guardada en: {csv_path}")
print(nota_minima_df.to_string(index=False))
