...
csv_path = os.path.join(script_dir, 'alumnos.csv')
salida_path = os.path.join(script_dir, 'alumnos_honor.csv')
...
df['Grupo de Honor'] = df['Promedio General'] > 9

df.to_csv(salida_path, index=False)
print(f"CSV con columna 'Grupo de Honor' guardado en: {salida_path}")
print(df[['Nombre', 'Promedio General', 'Grupo de Honor']].to_string(index=False))
